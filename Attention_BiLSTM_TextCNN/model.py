...
class TextCNN():
    def __init__(self, dataset, filter_numbers, sentence_length, title_length, lstm_n_hidden, lstm_n_layer,
                 cnn_layer_size_list,
        ...
        self.name = str(filter_numbers) + '_' + str(lstm_n_hidden) + '+' + str(cnn_layer_size_list) + '+' + str(
            learn_rate) + '_' + str(batch_size) + '_' + str(dropout_pro) + '+attention'
        self.name = self.name.replace('[', '').replace(']', '')
        self.learning_rate = learn_rate
        self.epochs = epochs
        self.seq_length = sentence_length + title_length
        ...
        self.input_zh_title = tf.placeholder(tf.int32, shape=[None, title_length])
        self.input_pos_title = tf.placeholder(tf.int32, shape=[None, title_length])
        self.input_neg_title = tf.placeholder(tf.int32, shape=[None, title_length])
        ...
        '''
        3. inite the c(i.e. title word embedding) to attention based model 
        '''
        with tf.name_scope('attention_inital'):
            embed_zh_title = tf.nn.embedding_lookup(self.zh_dict_vec, self.input_zh_title)
            embed_pos_title = tf.nn.embedding_lookup(self.en_dict_vec, self.input_pos_title)
            embed_neg_title = tf.nn.embedding_lookup(self.en_dict_vec, self.input_neg_title)

        '''
        4. attention based layer
        '''
        # TODO 完善attention Model
        w_c_zh = tf.Variable([self.embedding_dim, self.embedding_dim], name='w_c_zh', dtype=tf.float32)
        w_c_zh = weight_variable([self.embedding_dim, self.embedding_dim])
        w_y_zh = weight_variable([self.embedding_dim, self.embedding_dim])
        b_zh = bias_variable([self.embedding_dim])
        m_zh = tf.nn.tanh(tf.add(tf.concat([tf.tensordot(embed_zh_title, w_c_zh, axes=1)
                                               , tf.tensordot(embeddings_zh, w_y_zh, axes=1)], axis=1), b_zh))
        softmax_zh = tf.nn.softmax(m_zh)
        reshape_embeddding_zh = tf.concat([embeddings_zh, embed_zh_title], axis=1)
        # output_zh = tf.reduce_sum(tf.multiply(reshape_embeddding_zh,softmax_zh), 1)
        output_zh = tf.reshape(tf.multiply(reshape_embeddding_zh, softmax_zh),
                               [-1, self.seq_length, self.embedding_dim])

        w_c_en = weight_variable([self.embedding_dim, self.embedding_dim])
        w_y_en = weight_variable([self.embedding_dim, self.embedding_dim])
        b_en = bias_variable([self.embedding_dim])
        m_pos = tf.nn.tanh(tf.add(tf.concat([tf.tensordot(embed_pos_title, w_c_en, axes=1)
                                                , tf.tensordot(embeddings_pos, w_y_en, axes=1)], axis=1), b_en))
        m_neg = tf.nn.tanh(tf.add(tf.concat([tf.tensordot(embed_neg_title, w_c_en, axes=1)
                                                , tf.tensordot(embeddings_neg, w_y_en, axes=1)], axis=1), b_en))
        softmax_pos = tf.nn.softmax(m_pos)
        softmax_neg = tf.nn.softmax(m_neg)
        reshape_embeddding_pos = tf.concat([embeddings_pos, embed_pos_title], axis=1)
        reshape_embeddding_neg = tf.concat([embeddings_neg, embed_neg_title], axis=1)

        # output_pos = tf.reduce_sum(tf.multiply(reshape_embeddding_pos,softmax_pos), 1)
        # output_neg = tf.reduce_sum(tf.multiply(reshape_embeddding_neg,softmax_neg), 1)

        output_pos = tf.reshape(tf.multiply(reshape_embeddding_pos, softmax_pos),
                                [-1, self.seq_length, self.embedding_dim])
        output_neg = tf.reshape(tf.multiply(reshape_embeddding_neg, softmax_neg),
                                [-1, self.seq_length, self.embedding_dim])

        '''
        5. use BiLSTM to zh and pos/neg 
        ...
        '''
        6. concat zh outputs and pos/neg outputs
        '''
        ...
        '''
        7. conv layer + maxpool layer for each filer size
        '''
        pool_layer_list_pos = []
        pool_layer_list_neg = []
        W_list = []
        ...
        '''
        8. full connect droput + softmax + l2
        '''
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            feed = {
                ...
                self.input_zh_title: train_batch[5][6],
                self.input_pos_title: train_batch[5][7],
                self.input_neg_title: train_batch[5][8],
    ...
    def val(self, epoch):
        ...
        for j in range(len(val_batch_data)):
            score = self.session.run(self.score_pos, feed_dict={
                ...
                self.input_zh_title: val_abs_data[5][6],
                self.input_pos_title: val_abs_data[5][7],
                self.input_neg_title: val_abs_data[5][8],
