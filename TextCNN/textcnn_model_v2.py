...
class TextCNN():
    ...
    def __init__(self, dict_vec, num_classes, shuffer_falg, static_falg, filter_numbers, filter_sizes, sentence_length, embedding_size,
        ...
        with self.train_graph.as_default():
            ...
            with tf.name_scope('embedding_layer'):
                ...
                # pdb.set_trace()
                self.embedding_layer_W = tf.Variable(initial_value=dict_vec, dtype=tf.float32, trainable=train_bool,
                                                     name='embedding_layer_W')
                print("ssssssss")
            ...
            with tf.name_scope('soft_max_layer'):
                ...
                self.predictions = tf.argmax(self.softmax_values, axis=1, name='predictions')
            ...
            with tf.name_scope('accuracy'):
                correct_acc = tf.equal(self.predictions, tf.argmax(self.input_y, axis=1))
    ...
    def __add_conv_layer(self, filter_size, filter_num):
        with tf.name_scope('conv-maxpool-size%d' % (filter_size)):
            ...
            print('str(self.embedding_layer_layer     '+str(self.embedding_layer_layer))
            print('self.embedding_layer_expand:      '+str(self.embedding_layer_expand))
