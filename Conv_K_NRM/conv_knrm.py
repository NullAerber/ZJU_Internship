...
W_conv1 = tf.Variable(tf.truncated_normal([1, embedding_dim, 1, filter_numbers], stddev=0.1), name="W_conv1")
b_conv1 = tf.Variable(tf.constant(0.1, shape=[filter_numbers]), name="b_conv1")

with tf.variable_scope("conv1", reuse=None) as scope:
    conv1 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_1, -1), W_conv1, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv1")
    conv_output1 = tf.nn.dropout(tf.nn.bias_add(conv1, b_conv1, name="conv_output1"), dropout_pro)
with tf.variable_scope("conv1", reuse=True) as scope:
    conv2 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_2, -1), W_conv1, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv2")
    conv_output2 = tf.nn.dropout(tf.nn.bias_add(conv2, b_conv1, name="conv_output2"), dropout_pro)
with tf.variable_scope("conv1", reuse=True) as scope:
    conv3 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_3, -1), W_conv1, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv3")
    conv_output3 = tf.nn.dropout(tf.nn.bias_add(conv3, b_conv1, name="conv_output3"), dropout_pro)
conv_zh.append(conv_output1)
conv_en.append(conv_output2)
conv_ne.append(conv_output3)

W_conv2 = tf.Variable(tf.truncated_normal([2, embedding_dim, 1, filter_numbers], stddev=0.1), name="W_conv2")
b_conv2 = tf.Variable(tf.constant(0.1, shape=[filter_numbers]), name="b_conv2")

with tf.variable_scope("conv1", reuse=None) as scope:
    conv1 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_1, -1), W_conv2, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv1")
    conv_output1 = tf.nn.dropout(tf.nn.bias_add(conv1, b_conv2, name="conv_output1"), dropout_pro)
with tf.variable_scope("conv1", reuse=True) as scope:
    conv2 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_2, -1), W_conv2, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv2")
    conv_output2 = tf.nn.dropout(tf.nn.bias_add(conv2, b_conv2, name="conv_output2"), dropout_pro)
with tf.variable_scope("conv1", reuse=True) as scope:
    conv3 = tf.nn.conv2d(tf.expand_dims(pretrained_embeddings_3, -1), W_conv2, strides=[1, 1, embedding_dim, 1],
                         padding="SAME", name="conv3")
    conv_output3 = tf.nn.dropout(tf.nn.bias_add(conv3, b_conv2, name="conv_output3"), dropout_pro)
conv_zh.append(conv_output1)
conv_en.append(conv_output2)
conv_ne.append(conv_output3)
...
feats1 = []
feats2 = []
mus = kernal_mus(n_bins, use_exact=True)
sigmas = kernel_sigmas(n_bins, lamb, use_exact=True)
# nodes to hold mu sigma
mu = tf.reshape(input_mu, shape=[1, 1, n_bins])
sigma = tf.reshape(input_sigma, shape=[1, 1, n_bins])
...
# Learning-To-Rank layer. o is the final matching score.
W1 = weight_variable([n_bins * hmax * hmax, 1])
b1 = bias_variable([1])
